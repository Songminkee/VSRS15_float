...
void CViewInterpolation1D::PixelMapping(int x, int y, unsigned char d, float z, unsigned char* RefY, unsigned char* RefU, unsigned char* RefV, int ViewId, double dk, int flooring)
{
       ...
       y2<Height              && y2>=0    ) // (x2,y2) must fall within the picture
  {
	  ...
	  //std::cout << "Dmap[ViewId][ind2]===" << Dmap[ViewId][ind2] << std::endl;
    nearToCamera = false;
    if (Mask[ViewId][ind2]==NOTHOLEPIXEL)
    {
		std::cout << "Zmap[ViewId][ind2]" << Zmap[ViewId][ind2] << std::endl;
#if USE_ZMAP
...
void CViewInterpolation1D::ForwardWarpSingleView(int ViewId)
{
  ...
  std::cout << "WarpEnhancementOption" << WarpEnhancementOption << std::endl;
  ...
  if( WarpToRight == 0) // left view-- from left 2 right
    for (y = 0; y < Height; y++)
      for (x = 0; x < Width2;  x++)
      {
		...
		std::cout << "Znear[ViewId]" << Znear[ViewId] << std::endl;
...
void CViewInterpolation1D::ForwardWarp() // unsigned char* RefLeft, unsigned char* RefRight, unsigned char* RefDepthLeft, unsigned char* RefDepthRight
{
  ...
  std::cout << "Width2=" << Width2 << std::endl;
  std::cout << "Height=" << Height << std::endl;
  std::cout << "SubPelOption=" << SubPelOption << std::endl;
  ...
  std::cout << "SplattingOption" << SplattingOption << std::endl;
  std::cout << "CleanNoiseOption" << CleanNoiseOption << std::endl;
...
int CViewInterpolation1D::DoOneFrame(ImageType* RefLeft, ImageType* RefRight, DepthType* RefDepthLeft, DepthType* RefDepthRight, ImageType* Syn)
{
...
#endif
  printf("TemporalImproventOption = %d", TemporalImprovementOption);
  //Zhejiang, Temporal Improvement Option
  if(TemporalImprovementOption == 1)
  {
	  // 이전 프레임에서 얼마나 이동했는지 적용
  ...
  printf("UpsampleRefs = %d", UpsampleRefs);
...
void CViewInterpolation1D::FindDepthMaxMin( DepthType* map, int ViewID)
{
  ...
  Minz = MAX_DEPTH-1;
	...
	std::cout << "MaxTypeValue<DepthType>()" << MaxTypeValue<DepthType>() << std::endl;
	std::cout << "Maxz" << Maxz << std::endl;
	std::cout << "Minz" << Minz << std::endl;
	std::cout << "ViewID" << ViewID << std::endl;
	std::cout << "FocalLength" << FocalLength << std::endl;
	std::cout << "LTranslation" << LTranslation[ViewID] << std::endl;
	std::cout << "duPrincipal" << duPrincipal[ViewID] << std::endl;
  ...
  std::cout << "SplattingDepthThreshold" << SplattingDepthThreshold << std::endl;
}
