...
private:
  ...
  void    split_extrinsic_trans      (CvMat *exMat_dst_R, CvMat *exMat_dst_T,CvMat *exMat_dst, CvMat *exMat_src);
  ...
  bool	  init_camera_param_Mono(double Mat_In_Ref[9], double Mat_Ex_Ref[9], double Mat_Trans_Ref[3], double Mat_In_Vir[9], double Mat_Ex_Vir[9], double Mat_Trans_Vir[3],
	  CvMat *mat_in[2], CvMat *mat_ex_c2w[2], CvMat *mat_proj[2]);
  ...
  bool    init_3Dwarp_Mono(double Z_near, double Z_far, unsigned int uiDepthType,
	  const char *strCamParamFile, const char *strRefCamID, const char *strVirCamID,
	  double Mat_In_Ref[9], double Mat_Ex_Ref[9], double Mat_Trans_Ref[3],
	  double Mat_In_Vir[9], double Mat_Ex_Vir[9], double Mat_Trans_Vir[3]);
  ...
  void	  makeHomography_Mono   (CvMat *&matH_F2T, CvMat *matIN_from, CvMat *matEX_c2w_from, CvMat *matProj_to);
#else
  ...
  bool    depthsynthesis_3Dwarp_Mono(DepthType **pDepthMap, ImageType ***src);
  ...
  bool    viewsynthesis_reverse_3Dwarp_Mono(ImageType ***src, DepthType **pDepthMap, int th_same_depth);
